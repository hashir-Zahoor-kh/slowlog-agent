...
def run_init() -> int:
    ...
    _write_config(
        log_group_name=log_group_name, aws_region=region, aws_profile=profile, db_dsn=db_dsn
    )
...
def _check_claude_binary() -> None:
    if shutil.which("claude"):
        click.secho("\n`claude` binary found on PATH.", fg="green")
        return
    click.secho("\n`claude` binary not found on PATH.", fg="yellow")
    click.echo("Install it with:\n  npm install -g @anthropic-ai/claude-code")
    click.echo("(or see https://docs.claude.com/claude-code for other install options)\n")


def _write_config(
    *, log_group_name: str, aws_region: str, aws_profile: str, db_dsn: str | None
) -> None:
    lines = [
